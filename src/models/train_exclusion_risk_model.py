...
# Try importing SHAP; fall back to model feature attributions if not installed
try:
    import shap
    HAS_SHAP = True
except ImportError:
    HAS_SHAP = False
    print("[note] 'shap' package not installed. Using model feature importance attribution fallback.", flush=True)

# Try importing XGBoost; gracefully fall back to Scikit-Learn if OpenMP/libomp is missing
USE_XGBOOST = False
try:
    import xgboost as xgb
    # Trigger dylib initialization test
    xgb.XGBRegressor()
    USE_XGBOOST = True
...
MODEL_JOBLIB = ROOT / "data" / "processed" / "exclusion_risk_model.joblib"
...
def train():
    ...
    # Feature attribution computation (using SHAP if available, otherwise relative feature contributions)
    if HAS_SHAP:
        explainer = shap.TreeExplainer(model)
        shap_values = explainer(X)
        vals = shap_values.values if hasattr(shap_values, "values") else shap_values
        shap_df = pd.DataFrame(vals, columns=FEATURE_COLUMNS)
    else:
        # Standardized feature attribution based on model feature importances & normalized deviations
        importances = getattr(model, "feature_importances_", np.ones(len(FEATURE_COLUMNS)) / len(FEATURE_COLUMNS))
        std_devs = X.std().replace(0, 1.0)
        norm_X = (X - X.mean()) / std_devs
        contribs = norm_X.values * importances
        shap_df = pd.DataFrame(contribs, columns=FEATURE_COLUMNS)
