...
|---|---|---|---|
| UIDAI Public Data Portal (`data.uidai.gov.in`) | District-wise Aadhaar enrolment saturation %, updated weekly | REST API, no key needed: `https://data.uidai.gov.in/uiddatacatalog/rest/{datasetcode}` | Saturation ≠ authentication failure. Use as a base layer / proxy for digital-ID coverage, not as your main label. |
